...
namespace PlateauToolkit.Rendering.Editor
{
    ...
    class AutoTexturing
    {
        public bool m_OnlyProcessLod1_Blds = true;
        ...
        void OptimizeProcessDelayCall(List<GameObject> selectedObjects)
        {
            ...
            EditorApplication.delayCall += () =>
            {
                for (int i = 0; i < selectedObjects.Count; i++)
                {
                    ...
                    // Check if the GameObject already has a Processed component
                    if (selectedObject.GetComponent<Processed>() != null)
                    {
                        // Skip this GameObject if it already has a Processed component
                        continue;
                    }

                    if (meshRenderer != null && meshFilter != null)
                    {
                        string lodLevel = PlateauRenderingBuildingUtilities.GetLODLevel(selectedObject);
                        // generally we only want to run the process below on game objects that have bldg in their name and their parent contains the string LOD1
                        if (!m_OnlyProcessLod1_Blds || (lodLevel == "LOD1" || lodLevel == "LOD1 - Separated"))
                        {
                            GameObject lod2Object = PlateauRenderingBuildingUtilities.FindSiblingLodObject(selectedObject, "LOD2");

                            //When LOD1 does not have LOD2
                            if (lod2Object == null)
                            {
                            ...
                            {
                                string lod2State = PlateauRenderingBuildingUtilities.GetLODLevel(lod2Object);
                                if (lod2State == "LOD2")
                                {
                                ...
                                else if (lod2State == "LOD2 - Separated")
                                {
                        ...
                        else if (!m_OnlyProcessLod1_Blds || lodLevel == "LOD1 - Combined")
                        {
                        ...
                        // generally we only want to run the process below on game objects that have bldg in their name and their parent contains the string LOD2
                        else if (!m_OnlyProcessLod1_Blds || lodLevel == "LOD2")
                        {
                            ProcessLod2(selectedObject, meshRenderer, meshFilter);
                        }

                        else if (!m_OnlyProcessLod1_Blds || lodLevel == "LOD2 - Combined")
                        {
                            List<GameObject> newGameObjects = PlateauRenderingMeshUtilities.SeparateSubmesh(selectedObject);
                        ...
                        else if (!m_OnlyProcessLod1_Blds || lodLevel == "LOD2 - Separated")
                        {
                    ...
                    {
                        // when the building object does not have mesh renderers and mesh filters they are LOD2 Separated mesh
                        GameObject combinedObj = PlateauRenderingBuildingUtilities.CombineSeparatedLOD2(selectedObject);
                        MeshRenderer newMeshRenderer = combinedObj.GetComponent<MeshRenderer>();
                        MeshFilter newMeshFilter = combinedObj.GetComponent<MeshFilter>();
                        ProcessLod2(combinedObj, newMeshRenderer, newMeshFilter);
        ...
        void ProcessNewLOD1(GameObject go, MeshRenderer meshRenderer, MeshFilter meshFilter, GameObject targetObject, GameObject referenceObject)
        {
            ...
            PlateauRenderingBuildingUtilities.SetBuildingVetexColorForWindow(newLOD1Mesh, boundingBox, go);
        ...
        void ProcessLod2(GameObject go, MeshRenderer meshRenderer, MeshFilter meshFilter)
        {
            ...
            PlateauRenderingBuildingUtilities.SetBuildingVetexColorForWindow(meshFilter.sharedMesh, boundingBox, go);
